...
:: ============================================================================
set "ESCAPE="
for /f "tokens=1 delims==" %%a in ('"prompt $H"') do set "ESCAPE=%%a"
if defined ESCAPE if "%ESCAPE%" neq "" (
    <nul set /p "=%ESCAPE%" > "%TEMP%\ansienable.vbs"
    echo Set objShell = CreateObject("WScript.Shell") >> "%TEMP%\ansienable.vbs"
    echo objShell.Run "cmd /c exit", 0, True >> "%TEMP%\ansienable.vbs"
    for /f %%b in ('cscript //nologo "%TEMP%\ansienable.vbs"') do set "ESCAPE=%%b"
    del "%TEMP%\ansienable.vbs" 2>nul
)
REM Alternative: Enable Virtual Terminal via registry
